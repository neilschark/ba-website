...
test_type = "database_int_250"
deployments = ["classic", "docker", "orchestration"]
save_filename = f"{timestamp}__{test_type}_latency"
average_data = {}

# Get data from csv
data = extract_data_csv(deployments, timestamp, test_type, x_position=0, y_position=14, calc_average=False)
data_table_data = extract_data_csv(deployments, timestamp, test_type, x_position=1, y_position=14, calc_average=False)
...
plt.title("Datenbank (Integer): Latenz der Antwort (99,9%ile, ms) 250 Nutzer")
